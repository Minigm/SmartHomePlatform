...
public class act_CO2Sensor extends AppCompatActivity {
    //记录状态变量
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chart = (LineChart) findViewById(R.id.chart_co2sensor);
        receiver = new mReceiver();
        IntentFilter filter = new IntentFilter(GMCompanyDeviceHelper.getBroadcastName);
        act_CO2Sensor.this.registerReceiver(receiver,filter);
        LineDataSet dataSet = new LineDataSet(dataFlow.getEntries(),"近10次测量数据");
        dataSet.setColor(Color.BLACK);
        Description description = chart.getDescription();
        description.setText("CO2浓度值");
        LineData lineData = new LineData(dataSet);
        chart.setData(lineData);
        chart.invalidate();
        ((TextView)findViewById(R.id.text_state_co2sensor)).setText("状态：off");
    }

    @Override
    protected void onDestroy() {
        act_CO2Sensor.this.unregisterReceiver(receiver);
        super.onDestroy();
    }

    public void buttonBack_co2sensorOnClick(View view) {
        finish();
    }

    public void buttonOn_co2sensorOnClick(View view) {
        MQTTManager mqttManager = new MQTTManager("CO2Sensor"+id,"","");
        mqttManager.publish("Commond",id+";state;on;");
        mqttManager.disconnect();
    }

    public void buttonOff_co2sensorOnClick(View view) {
        MQTTManager mqttManager = new MQTTManager("CO2Sensor"+id,"","");
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent != null){
                ...
                if (id_in.equals(id)) {
                    ...
                    ((TextView)findViewById(R.id.text_state_co2sensor)).setText("状态："+state);
