...
public class act_GasSensor extends AppCompatActivity {
    //记录状态变量
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.lay_gassensor);
        Intent intent = getIntent();
        id = intent.getStringExtra("id");
        chart = (LineChart) findViewById(R.id.chart_gassensor);
        receiver = new mReceiver();
        IntentFilter filter = new IntentFilter(GMCompanyDeviceHelper.getBroadcastName);
        act_GasSensor.this.registerReceiver(receiver,filter);
        LineDataSet dataSet = new LineDataSet(dataFlow.getEntries(),"近10次测量数据");
        dataSet.setColor(Color.BLACK);
        Description description = chart.getDescription();
        description.setText("可燃气体浓度值(mg/m³)");
        LineData lineData = new LineData(dataSet);
        chart.setData(lineData);
        chart.invalidate();
        ((TextView)findViewById(R.id.text_state_gassensor)).setText("状态：off");
    }

    @Override
    protected void onDestroy() {
        act_GasSensor.this.unregisterReceiver(receiver);
        super.onDestroy();
    }

    public void buttonBack_gassensorOnClick(View view) {
        finish();
    }

    public void buttonOn_gassensorOnClick(View view) {
        MQTTManager mqttManager = new MQTTManager("GasSensor"+id,"","");
        mqttManager.publish("Commond",id+";state;on;");
        mqttManager.disconnect();
    }

    public void buttonOff_gassensorOnClick(View view) {
        MQTTManager mqttManager = new MQTTManager("GasSensor"+id,"","");
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent != null){
                ...
                if (id_in.equals(id)) {
                    ...
                    ((TextView)findViewById(R.id.text_state_gassensor)).setText("状态："+state);
